...
bool PODEM_Generator::test_possible_check(std::vector<DF_NodePtr> &D_frontier)
{
    ...
    for (auto it:D_frontier)
        if (!look_ahead_check(it->df_gate))
            return 0;
...
// void PODEM_Generator::save_result(const string &cirName, const string &inDat) {
//     string s(35,'-');
//
//     ofstream outFile("./result/result_"+cirName, ios_base::app);
//     outFile<<s<<endl;
//
//     outFile<<outList;
//     outFile.close();
